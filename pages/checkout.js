...
function Checkout(props) {
  ...
  const uniqueid = router.query.renew;
  var couponparam = router.query.coupon;
...
let date=today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
 ...
 const changewtp = (e) => {
  ...
  const coupon = () => {
     ...
     if ( couponcode.toUpperCase() === "HAPPY20"){
      setDiscountPerc(20)
      setCouponValid(true)
     }
     else   if ( couponcode.toUpperCase() === "5OFF"){
      setDiscountPerc(5)
      setCouponValid(true)
     }
     else   if ( couponcode.toUpperCase() === "HAPPY10"){
      setDiscountPerc(10)
      setCouponValid(true)
     }
     else   if ( couponcode.toUpperCase() === "BEST20"){
      setDiscountPerc(20)
      setCouponValid(true)
     }
     else   if ( couponcode.toUpperCase() === "LAST25"){
      setDiscountPerc(25)
      setCouponValid(true)
     }
     else   if ( couponcode.toUpperCase() === "FAST50"){
      setDiscountPerc(50)
      setCouponValid(true)
     }
}
...
const toggleProxyCheck = () => {
  ...
  const pack3 = [Number.parseFloat(7.99*coeff).toFixed(2),Number.parseFloat(5.99*coeff).toFixed(2) ,Number.parseFloat(1.33*coeff).toFixed(2) ,Number.parseFloat(23.97*coeff).toFixed(2) ,Number.parseFloat(17.97*coeff).toFixed(2) ,Number.parseFloat(3.99*coeff).toFixed(2) ];

  const pack6 = [Number.parseFloat(6.29*coeff).toFixed(2),Number.parseFloat(3.99*coeff).toFixed(2) ,Number.parseFloat(1.29*coeff).toFixed(2) ,Number.parseFloat(37.74*coeff).toFixed(2) ,Number.parseFloat(23.94*coeff).toFixed(2) ,Number.parseFloat(7.74*coeff).toFixed(2) ];
  const pack12 = [Number.parseFloat(4.91*coeff).toFixed(2),Number.parseFloat(2.99*coeff).toFixed(2) ,Number.parseFloat(0.99*coeff).toFixed(2) ,Number.parseFloat(58.91*coeff).toFixed(2) ,Number.parseFloat(35.88*coeff).toFixed(2) ,Number.parseFloat(11.88*coeff).toFixed(2) ];
...
const Month12 = () => {
  ...
  const toggleEur = () => {
     ...
     setSymbol("€")
     setCoeff(1.17)
     setSvg("https://netflytv.com/svg/currency-flags/eur.svg")
   }    
   const toggleUsd = () => {
 
      setCurrency("USD")
      setSymbol("$")
      setSvg("https://netflytv.com/svg/currency-flags/usd.svg")
   ...
   return (
      <div className="font-press-start  ">
         ...
         <div id="app" className="bg-indigo-100  font-press " data-v-app="">
         <nav  className=" hidden bg-white border-b border-gray-200 fixed w-full z-40">
               <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                  <div className="flex justify-between h-16">
                     <div className="flex"><a href="/" className="flex-shrink-0 flex items-center">
                        
                        <h1 className="h-8 w-2/3 "  alt="Workflow" >Currency</h1>
                  ...
                  <div className="absolute z-50 mt-2 rounded-md shadow-lg w-36 origin-top-right right-0" >
                     <div className={toggleMenu ? 'hidden rounded-md ring-1 ring-black ring-opacity-5 py-1 bg-white': ' rounded-md ring-1 ring-black ring-opacity-5 py-1 bg-white'} 
                      >
                          <button onClick={ () => toggleEur() } className="block py-2 px-5 flex"><img className="h-6 rounded-md flex-shrink mr-2" src="https://netflytv.com/svg/currency-flags/eur.svg" alt="EUR Currency flag"/><span>{symbol} EUR</span></button>
          ...
          <div className="px-4 py-5 md:hidden">
            ...
            <div className="flex space-x-2 items-center">
              ...
              <p className="font-semibold package-price flex-shrink" style={{fontSize: '25px'}}><span className="font-medium" style={{fontSize: '14px'}}>{symbol}</span>{pack6[0]} <span className="font-medium" style={{fontSize: '14px'}}> / mo</span></p>
               ...
               <div ref={myRef} id="optional-addons"  className="flex space-x-4 items-center py-12 mt-6"><span className="text-white  h-7 px-3 rounded-lg inline-flex justify-center items-center" style={{background: 'rgb(55, 211, 72)', outline: 'none'}}>Step 2</span><span className="font-semibold md:text-2xl text-xl">Optional Addons
             
               </span></div>
               <div className="optional-addons space-y-4">


             


                  <div className=" shadow-md sm:flex items-center sm:space-x-4 sm:space-y-0 space-y-2 bg-white py-4 px-6 rounded-xl max-w-2xl cursor-pointer">
                     <div onClick={ () => toggleExtraCheck() } className="flex-1">
                        <h2 className="uppercase"> EXTRA CONNECTION </h2>
                        <p className="text-gray-500 text-sm">Use your IPTV on multiple devices at the same time

</p>
                     </div>
                     <div  className="flex-shrink flex justify-center items-center space-x-3"><span className="text-sm">{symbol} {extra} / month</span>
                     <button onClick={ () => toggleExtraCheck() } type="button" 
                     
                     className="flex-shrink-0 group relative rounded-full inline-flex items-center justify-center h-5 w-10 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" role="switch" aria-checked="false"><span className="sr-only">Use setting</span><span aria-hidden="true" className="pointer-events-none absolute bg-white w-full h-full rounded-md"></span>
                     
                     <span 
                     className = {extrachecked ? 'bg-green-600 pointer-events-none absolute h-4 w-9 mx-auto rounded-full transition-colors ease-in-out duration-200' : 'bg-gray-200 pointer-events-none absolute h-4 w-9 mx-auto rounded-full transition-colors ease-in-out duration-200'}
                      aria-hidden="true"></span>
                     
                     <span 
                     className = {extrachecked? 'translate-x-5 pointer-events-none absolute left-0 inline-block h-5 w-5 border border-gray-200 rounded-full bg-white shadow transform ring-0 transition-transform ease-in-out duration-200' : 'translate-x-0 pointer-events-none absolute left-0 inline-block h-5 w-5 border border-gray-200 rounded-full bg-white shadow transform ring-0 transition-transform ease-in-out duration-200'}
                      aria-hidden="true"></span></button></div>
                  </div>


            <div className="shadow-md sm:flex items-center sm:space-x-4 sm:space-y-0 space-y-2 bg-white py-4 px-6 rounded-xl max-w-2xl cursor-pointer">
                     <div onClick={ () => toggleProxyCheck() } className="flex-1">
                        <h2 className="uppercase"> Proxy Protection <span className="bg-green-400 text-white rounded-full uppercase px-2 ml-1 ">recommended</span> </h2>
                        <p className="text-gray-500 text-sm">Built-in proxy to hide the traffic to your ISP and protect your privacy

</p>
                     </div>
                     <div  className="flex-shrink flex justify-center items-center space-x-3"><span className="text-sm">{symbol} {proxyprice} / month</span>
                     <button onClick={ () => toggleProxyCheck() } type="button" 
                     
                     className="flex-shrink-0 group relative rounded-full inline-flex items-center justify-center h-5 w-10 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" role="switch" aria-checked="false"><span className="sr-only">Use setting</span><span aria-hidden="true" className="pointer-events-none absolute bg-white w-full h-full rounded-md"></span>
                     
                     <span 
                     className = {proxychecked ? 'bg-green-600 pointer-events-none absolute h-4 w-9 mx-auto rounded-full transition-colors ease-in-out duration-200' : 'bg-gray-200 pointer-events-none absolute h-4 w-9 mx-auto rounded-full transition-colors ease-in-out duration-200'}
                      aria-hidden="true"></span>
                     
                     <span 
                     className = {proxychecked? 'translate-x-5 pointer-events-none absolute left-0 inline-block h-5 w-5 border border-gray-200 rounded-full bg-white shadow transform ring-0 transition-transform ease-in-out duration-200' : 'translate-x-0 pointer-events-none absolute left-0 inline-block h-5 w-5 border border-gray-200 rounded-full bg-white shadow transform ring-0 transition-transform ease-in-out duration-200'}
                      aria-hidden="true"></span></button></div>
                  </div>

                  <div className="shadow-md sm:flex items-center sm:space-x-4 sm:space-y-0 space-y-2 bg-white py-4 px-6 rounded-xl max-w-2xl cursor-pointer">
                     <div onClick={ () => toggleAdult() } className="flex-1">
                        <h2 className="uppercase"> Adult channels / XXX Channels <span className="bg-green-400 rounded-full text-white px-2 ml-1 ">FREE</span> </h2>
                        <p className="text-gray-500 text-sm">Adult channels are channels that feature adult content, in the sense of pornographic

</p>
                     </div>
                     <div  className="flex-shrink flex justify-center items-center space-x-3"><span className="text-sm">{symbol} 0.00 / month</span>
                     <button onClick={ () => toggleAdult() } type="button" 
                     
                     className="flex-shrink-0 group relative rounded-full inline-flex items-center justify-center h-5 w-10 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" role="switch" aria-checked="false"><span className="sr-only">Use setting</span><span aria-hidden="true" className="pointer-events-none absolute bg-white w-full h-full rounded-md"></span>
                     
                     <span 
                     className = {adult ? 'bg-green-600 pointer-events-none absolute h-4 w-9 mx-auto rounded-full transition-colors ease-in-out duration-200' : 'bg-gray-200 pointer-events-none absolute h-4 w-9 mx-auto rounded-full transition-colors ease-in-out duration-200'}
                      aria-hidden="true"></span>
                     
                     <span 
                     className = {adult? 'translate-x-5 pointer-events-none absolute left-0 inline-block h-5 w-5 border border-gray-200 rounded-full bg-white shadow transform ring-0 transition-transform ease-in-out duration-200' : 'translate-x-0 pointer-events-none absolute left-0 inline-block h-5 w-5 border border-gray-200 rounded-full bg-white shadow transform ring-0 transition-transform ease-in-out duration-200'}
                      aria-hidden="true"></span></button></div>
                  </div>
               ...
               <div  ref={myRef2} id="fill-details" className="flex space-x-4 items-center py-12 mt-6"><span className="text-white h-7 px-3 rounded-lg inline-flex justify-center items-center"style={{background: 'rgb(55, 211, 72)', outline: 'none'}} >Step 3</span><span className="font-semibold md:text-2xl text-xl">Fill Your Details</span></div>
               <div className="fill-details grid grid-cols-3 gap-10">
                  <div className="shadow-md col-span-3 order-2 md:col-span-2 md:order-1 items-center bg-white p-4 md:p-8 rounded-xl">
                     <div className="space-y-7">
                        <div className={uniqueid == null ? " space-y-3" : "space-y-3 hidden"} >
                           <div>
                           ...
                           <div>
                              <div className=" rounded-md px-3   "><label htmlFor="wtp" className="block  font-medium text-gray-900">Whatsapp Number <span className="text-green-600">Optional</span></label><input type="tel" 
                             
                              value={wtp}
                             onChange={changewtp}
                              name="phone" id="phone" className="appearance-none block w-full bg-grey-lighter text-grey-darker border-2 border-grey-lighter rounded-lg h-10 px-4" placeholder="Example : +44 795216015" /></div>
                           </div>
...
<option value="Any" >Any Other Device</option>
...
<input 
 ...
 placeholder="00:1A:79:XX:XX:XX" className="appearance-none block w-full bg-grey-lighter text-grey-darker border-2 border-grey-lighter rounded-lg h-10 px-4" type="text" name="mac"  style={{outline: 'none'}} />
</div>
                           </div>
                        </div>
   ...
   <div class="space-x-4 ">
                           ...
                           ><span className="inline-block flex-1"> <strong> {couponcode} </strong> Coupon {discountperc}% Discount   <Icon className="inline-block ml-1 h-6 w-6" icon="twemoji:smiling-face-with-heart-eyes" color="#FF0000" /></span>
                        ...
                        <section class="text-gray-700"><div class="grid lg:grid-cols-2 gap-4"><div class="space-y-4"><details class="bg-gray-100 rounded-md py-2 px-4"><summary class="font-semibold cursor-pointer">What is EXTRA CONNECTION Option?</summary><span class="pt-2 text-sm text-gray-500 inline-block">1 subscription can be used on multiple devices but you can watch only on 1 device at the same time.
                        If you want to use the same subscription on 2 devices at the same you'll need to check this option .</span></details><details class="bg-gray-100 rounded-md py-2 px-4"><summary class="font-semibold cursor-pointer">How can I install IPTV on my device?</summary><span class="pt-2 text-sm text-gray-500 inline-block">Check the installation tutorials <a class="underline text-green-500" href="/blog/how-to-install-iptv-in-my-device">here</a>, if there is any issue, feel free to contact us via whatsapp or telegram</span></details></div><div class="space-y-4"><details class="bg-gray-100 rounded-md py-2 px-4"><summary class="font-semibold cursor-pointer">What is PROXY PROTECTION Option?</summary><span class="pt-2 text-sm text-gray-500 inline-block">IPTV is illegal in some countries , By checking this option , we hide your IP & geolocation from your Internet Provider by issuing a virtual location.</span></details></div></div></section>
                  ...
                  <div className="col-span-3 order-2 md:col-span-1 md:order-2 items-center space-y-5">   <div className="shadow-md bg-white py-4 px-6 rounded-xl cursor-pointer  md:block">
                        <h2 className="text-xl font-semibold mb-2">Your Plan Includes:</h2>
                        <ul className="pl-3 leading-8">
                       <li>
                       Access to over 12,913 TV Channels
</li>
          <li>Thousands of Movies & Series
</li>
          <li>Easy Setup on All your Devices</li>
